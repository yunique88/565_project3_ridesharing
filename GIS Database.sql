...
CREATE TABLE IF NOT EXISTS `gisdatabase`.`CoordsTable` (
  `Coords` POINT NULL)
ENGINE = InnoDB
...
DELIMITER $$
USE `gisdatabase`$$
CREATE FUNCTION `computeCostModel` (ls linestring, startIndex INT, endIndex INT)
RETURNS Double
BEGIN
DECLARE Point1, Point2 Point;
DECLARE distance, xPoint1, xPoint2, yPoint1, yPoint2, distanceX, distanceY Double;
